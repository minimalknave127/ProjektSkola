...
namespace BookKeeperBECommon.Repos
{
    ...
    /// </summary>
    public class InvoiceItemRepoMysql
    {
        ...
        public IList<InvoiceItem> GetList()
        {
            using (var context = new MysqlContext())
            {
                ...
                var query = from u in context.InvoiceItem
                            select u;
                //var query = context.Users;
                var users = query.ToList<InvoiceItem>();
        ...
        public IList<InvoiceItem> FindList(InvoiceItem user)
        {
            using (var context = new MysqlContext())
            {
                ...
                IQueryable<InvoiceItem> query = BuildQuery(context.InvoiceItem, user);
        ...
        /// </summary>
        /// <param name="item">User to check the repo for.</param>
        /// <returns>Returns true :-: the user exists, false :-: the user does not exist.</returns>
        public bool Exists(InvoiceItem item)
        {
            using (var context = new MysqlContext())
            {
                ...
                IQueryable<InvoiceItem> query = BuildQuery(context.InvoiceItem, item);
        ...
        public InvoiceItem Load(InvoiceItem user)
        {
            if (!Exists(user))
            {
            ...
            using (var context = new MysqlContext())
            {
                ...
                return context.InvoiceItem.Find(user.ID);
        ...
        public void Store(InvoiceItem user)
        {
            using (var context = new MysqlContext())
            {
        ...
        public void Add(InvoiceItem user)
        {
            using (var context = new MysqlContext())
            {

                context.InvoiceItem.Add(user);
        ...
        public void Remove(InvoiceItem user)
        {
            using (var context = new MysqlContext())
            {
        ...
        private IQueryable<InvoiceItem> BuildQuery(IQueryable<InvoiceItem> query, InvoiceItem item)
        {
